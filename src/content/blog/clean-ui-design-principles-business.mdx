...
**Contrast is non-negotiable.** The [Web Content Accessibility Guidelines (WCAG)](https://www.w3.org/WAI/standards-guidelines/wcag/) require a 4.5:1 contrast ratio for body text and 3:1 for large text. This is not just a legal consideration - low-contrast text causes visitors to move on rather than struggle to read. Text that is hard to read is text that does not get read.
...
Minimum touch target size is 44x44 pixels - [Apple](https://developer.apple.com/design/human-interface-guidelines/) and [Google](https://m3.material.io/) both specify this. Smaller tap targets cause mis-taps, frustration, and lost conversions. Check every interactive element on your site on a real device, not just in Chrome's device emulator.
